...
def run_simulation(PATH_PARAMETERS, N_SIMULATION, schedule, D_start):
    NAME = 'simple_example'
    makespans = []
    critical_star, stds_star, n_activties = 0, [], 0
    for i in range(0, N_SIMULATION):
        ...
        if not schedule:
            critical_star, stds_star, n_activties = find_critical_path_duplicate(path, critical_star, stds_star, n_activties)
            print('N_SIMULATION ', len(makespans), critical_star, stds_star, n_activties)
    if not schedule:

...
import math
from process import CustomModel, CustomDataset
...
class Token(object):
    ...
    def predict_my_lstm(self):
        # case_id, act, res, month, day, hour
        #prefix = [[1, 7, 10, 2, 5, 12], [1, 176, 20, 2, 5, 13], [1, 179, 10, 2, 5, 14]]
        prefix_act_res_idx = []
        for row in self._prefix_lstm:
            key = str(row[1])+ '_' + str(row[2])
            if key in self._process.act_res_to_idx:
                prefix_act_res_idx.append(self._process.act_res_to_idx[key])
            else:
                prefix_act_res_idx.append(self._process.act_res_to_idx['UNKNOWN'])
                #print('UNKNOWN ASSIGNMENT', key)
            #prefix_act_res_idx.append(self.act_res_to_idx[key] if key in self.act_res_to_idx else 0)
        prefix_features = [row[3:] for row in self._prefix_lstm]

        # 2. Convert to tensor
        prefix_features_tensor = torch.tensor(prefix_features, dtype=torch.float32)
        prefix_act_res_idx_tensor = torch.tensor(prefix_act_res_idx, dtype=torch.long)

        # 3. Create padded sequences (ensure sequence length matches WINDOW_SIZE)
        padded_prefix_features = pad_sequences([prefix_features_tensor.tolist()], maxlen=self._process.WINDOW_SIZE, padding="pre",
                                               dtype="float32")
        padded_prefix_act_res_idx = pad_sequences([prefix_act_res_idx_tensor.tolist()], maxlen=self._process.WINDOW_SIZE,
                                                  padding="pre", dtype="int64")

        # Reshape to ensure the dimensions are correct
        padded_prefix_features = torch.tensor(padded_prefix_features, dtype=torch.float32)
        padded_prefix_act_res_idx = torch.tensor(padded_prefix_act_res_idx, dtype=torch.long)
        original_indices = [0]
        prediction_dataset = CustomDataset(
          padded_prefix_act_res_idx,  # Shape: (1, WINDOW_SIZE)
          padded_prefix_features,  # Shape: (1, WINDOW_SIZE, num_features)
          torch.tensor([0] * len(self._prefix_lstm), dtype=torch.float32),
          torch.tensor([0] * len(self._prefix_lstm), dtype=torch.float32),
          original_indices
        )
        prediction_loader = DataLoader(prediction_dataset, batch_size=1, shuffle=False)

        self._process.model.eval()
        with torch.no_grad():
          act_res_idx_input, features_input, mu_targets, sigma_targets, original_indices = next(iter(prediction_loader))

          # Forward pass through the model
          mu, sigma = self._process.model(act_res_idx_input, features_input)

          # mu and sigma are now two values returned from the model
    ...
    def predict_my_regressor(self):
        ...
        y_pred_single = self.loaded_model_reg.predict(array_reshaped)
        predicted_mu = y_pred_single[0, 0]
        predicted_sigma = y_pred_single[0, 1]
        proc = np.random.normal(predicted_mu, predicted_sigma, 1)[0]
        return max(0, proc)
